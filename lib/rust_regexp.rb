...
require_relative "rust_regexp/version"
...
class RustRegexp
  ...
  class << self
    ...
    # Regexp the flags (`/i`, `/x`, `/m`) are translated into a leading inline
    # group so the rust/regex engine sees them. Unsupported features
    # (lookaround, backrefs) still raise from the engine with a clear message.
  ...
  # Returns a `RustRegexp::MatchData` on hit, `nil` on miss — matching Ruby's
  # `Regexp#match` shape so the old `[]`-truthy-but-empty trap is gone.
  def match(haystack)
    _native_match(coerce_string(haystack))
  end

  # Case-equality. Enables `case/when` and RSpec's `expect(str).to match(re)`.
  def ===(other)
    return false unless other.respond_to?(:to_str)
    match?(other.to_str)
  end

  # Returns the byte offset of the first match, or nil. Matches the semantics
  # of `Regexp#=~` except positions are in **bytes** (rust/regex is byte-based).
  def =~(other)
  ...
  #
  # The string form uses rust/regex's native replacement template:
  # `$1`, `${name}`, `$$` for a literal `$`. To pass a replacement string that
  ...
  # `gsub(haystack, replacement)` or `gsub(haystack) { |m| ... }`. See `#sub`
